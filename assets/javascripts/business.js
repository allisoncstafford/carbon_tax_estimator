$(document).ready(function () {
  ...
  function updateModel() {
    ...
    model.setSalesTaxRate($(SALESTAXRATE).val());

    model.setBO($(BOTAXES).val());
    ...
    model.setAir(parseInt($("input[name=air]:checked").val()), $(SEATMILES).val(), $(AIRMONEY).val());
    ...
    model.setHome(parseInt(approxOrExactStatus.val()), parseInt(heating.val()), homeObject);
  ...
  function updateRadios() {
    ...
    if (parseInt($("input[name=air]:checked").val()) == 0) {
      $("#airOptionOne").show();
      $("#airOptionTwo").hide();
  ...
  function updateBoxesAndSpans() {
    ...
    setBox(SALESTAXRATE, model.salesTax.rate, model.error.salesTaxRate, model.errorMsg.salesTaxRate);
    setBox(BOTAXES, model.bo.taxes, model.error.bo, model.errorMsg.bo);
    ...
    setBox(SEATMILES, model.air.miles, model.error.airMiles, model.errorMsg.airMiles);
    setBox(AIRMONEY, model.air.money, model.error.airMoney, model.errorMsg.airMoney);
    ...
    $(BOSAVINGS).text(model.bo.savings);
...
function Model() {
  ...
  this.errorMsg = {
    ...
    salesTaxRate: "Please enter a valid tax rate",
    bo: "Please enter a valid B and O tax amount", 
    gas: "Please enter a valid number",
    airMiles: "Please enter a valid number of miles", 
    airMoney: "Please enter a valid dollar amount", 
...
Model.prototype.setSalesTax = function(tax) {
  this.salesTax.manualTaxes = tryMakeValidNumber(tax, 0);
  this.error.salesTax = this.setNumberError(this.salesTax.manualTaxes, this.error.salesTax);
}

Model.prototype.setSalesTaxRate = function(rate){
  this.salesTax.rate = tryMakeValidNumber(rate, 2);

  this.error.salesTaxRate = this.setNumberError(this.salesTax.rate, this.error.salesTaxRate);
  this.salesTax.savings = Math.round(this.salesTax.manualTaxes/this.salesTax.rate);
...
Model.prototype.setAir = function(option, seatMiles, airMoney){
  this.air.miles = tryMakeValidNumber(seatMiles, 0);
  this.air.money = tryMakeValidNumber(airMoney, 0);
  
  

  if (option == 0) {
    airTaxes = Math.round(this.air.miles/60*9.57/1000*25);
    this.error.airMiles = this.setNumberError(this.air.miles, this.error.airMiles);

  } else {
    airTaxes = Math.round(this.air.money/0.13/2/60*9.57/1000*25);
    this.error.airMoney = this.setNumberError(this.air.money, this.error.airMoney);
...
Model.prototype.setHome = function(approxOrExactStatus, heating, homeObject){
  ...
  if (!isNaN(approxOrExactStatus) && !(approxOrExactStatus == 1 && isNaN(heating))) {

    if (approxOrExactStatus == 0) {
      this.home.natGasTaxes = Math.round(5.306*25*this.home.therms/1000);
    } else {
      if (heating == 0 || heating == 4) {
        this.home.natGasTaxes = Math.round(5.306*25*732/1000*this.home.energyApproxPercent/100);
      } else {
        this.home.natGasTaxes = 0;
      }
    }
    ...
    if (approxOrExactStatus == 0) {
      this.home.fuelOilTaxes = Math.round(10.15*25*this.home.gallons/1000);
    } else {
      if (heating == 1) {
        this.home.fuelOilTaxes = Math.round(10.15*25*527/1000*this.home.energyApproxPercent/100);
      } else {
        this.home.fuelOilTaxes = 0;
      }
    }
    ...
    if (homeObject.utility != null) {
      ...
      if (approxOrExactStatus == 0) {
        this.home.elecTaxes = Math.round(centsPerKwh*this.home.kwh/100);
      } else {
        if (heating == 2) {
          this.home.elecTaxes = Math.round(centsPerKwh*(11000 + 11360)/100*this.home.energyApproxPercent/100);
        } else {
          this.home.elecTaxes = Math.round(centsPerKwh*(11000)/100*this.home.energyApproxPercent/100);
        }
      }
    }
